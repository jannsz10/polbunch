...
program bssim, eclass
	syntax, modstr(string) estimator(integer )cutoff(real) polynomial(integer) bw(real) h(integer) l(integer)  p(varname) obs(real) [b0(string) estopts(string) t0(numlist min=1 max=1  >=0 <=1) t1(numlist min=1 max=1  >=0 <=1) nl nlcom(string) log constant nopositiveshift boot notransform]
	preserve
	tempvar y
