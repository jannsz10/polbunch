		...
		* This program is part of the polbunch package.
		...
		program polbunch, eclass sortpreserve
			...
			quietly {
				...
				if `estimator'!=0 { //specify NL model
				...
				*********b0************
				...
				if `estimator'!=2 {
					forvalues k=1/`polynomial' {
						if "`log'"=="" loc cns `cns' - (`cutoff'^`=`k'+1')*((1+`shift')^(`=`k'+1')-`cutoff'^`=`k'+1')*({b`k'}/`=`k'+1')
				...
				**** bunch dummies ****
				...
				//TESTSTRING
				if "`test'"!="notest" {
					if `estimator'==3&"`log'"=="log" loc shiftres (exp((_b[/g`=`polynomial'-1']-_b[/b`=`polynomial'-1'])/(`polynomial'*_[/b`polynomial']))-1)
				...
				forvalues s=0/`stop' {
					...
					if "`saveunres'"!="" est sto `saveunres'
					}
