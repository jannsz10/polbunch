		...
		*/
		...
		program polbunch, eclass sortpreserve
			...
			NOIsily ///
			model(integer 3) /// Specify model - 3 = theoretically consistent efficient model, 2 = chetty, 1 = no adjustment, 0=data to the left only
			nodrop ///
			...
			quietly {
				if !inrange(`model',0,3) {
						noi di as error "Option model can take only values 1 (using data to the left only), 2 (no adjustment), 2 (Chetty et. al. adjustment) or 3 (theoretically consistent and efficient estimator)"
						exit 301
				}
				tempvar touse
				marksample touse
				...
				reg `y' 0.`dum'#(`rhsvars') 1.`dum'#(`rhsvars') 1.`dum' b0.`bunch'
				loc df_m_u=e(df_m)+1
				tempvar pred rss
				predict double `pred'
				...
				****** specify NL model string *****
				...
				if `model'!=2 {
					forvalues k=1/`polynomial' {
						...
						else loc cns `cns' - (((`cutoff'+`shift')^(`=`k'+1')-`cutoff'^`=`k'+1')*({b`k'}/`=`k'+1'))
						}
					if "`log'"=="" loc b0 (`cns')/(`cutoff'*`shift')
					else loc b0 (`cns')/`shift'
				}
				else {
					su `z'
					loc zmax=r(max)+`bw'/2
				...
				if `model'==0 loc modstr `b0'*(`z'<=`cutoff') + {g0}*(`z'>`cutoff')
				else if inlist(`model',1,2)|(`model'==3&"`log'"=="log") loc modstr `b0'
				...
				if `model'==2 loc modstr (1/(1+exp({lnshift})))*(`modstr')
				...
				************* Compute expressions for transformations ***********************
				...
				//gk parameters
				forvalues k=1/`=`polynomial'' {
					if `model'==0 loc nlcom `nlcom' (_b[/g`k'])
					else if `model'==1 loc nlcom `nlcom' (_b[/b`k'])
					else if `model'==2 loc nlcom `nlcom' (_b[/b`k']/(1+`shift'))
				...
				//g0
				if `model'==0 loc nlcom `nlcom' (_b[/g0])
				else if `model'==1 loc nlcom `nlcom' (`b0')
				else if `model'==2 loc nlcom `nlcom' ((`b0')/(1+`shift'))
				...
				*** INFERENCE: BINNED BOOTSTRAP OR ANALYTIC CLUSTERED STANDARD ERRORS ***
				if `bootreps'==0 {
					...
					if "`transform'"!="notransform" {
						...
						mat `b'=r(b)
						mat `V'=r(V)
						if `model'==2 { //compute marginal response and elasticity from solving integration constraint
							tempname h0
							mat `h0'=`b'
...
program bssim, eclass
	...
	ereturn post `b', obs(`obs')
				...
				loc nmiss=1
				while `nmiss'>0 {
					_rmcoll `y' 0.`dum'#(`rhsvars') 1.`dum'#(`rhsvars') 1.`dum' b0.`bunch' [fw=`fw']
