			...
			quietly {
				...
				if "`intconstraint'"=="nointconstraint" {
					...
					forvalues k=1/`polynomial' {
						loc cns `cns' - ((1+`shift')^`=`k'+1'-1)*({b`k'}/`=`k'+1')*((`cutoff'^`=`k'+1'))
					}
					loc b0 (`cns')/(`cutoff'*`shift')
					//if "`chetty'"=="chetty" loc modstr (`b0')/(1+(`varlist'>`cutoff')*exp({lnshift}))
					if "`log'"=="log" loc modstr `b0'
				...
				tokenize `names'
				forvalues k=1/`polynomial' {
					//if "`chetty'"=="chetty" loc modstr `modstr' +({b`k'}*`z'^`k')/(1+(`z'>`cutoff')*exp({lnshift}))
				...
				loc shift=subinstr("`shift'","{lnshift}","_b[/lnshift]",.)
				loc shift=subinstr("`shift'","{shift}","_b[/shift]",.)
				...
				forvalues k=1/`=`polynomial'' {
					if "`log'"=="" loc nlcom `nlcom' (g`k': _b[/b`k']*(1+`shift')^(`=`k'+1'))
						...
						loc str _b[/b`k']
						if (`polynomial'>`k') {
							forvalues n=1/`=`polynomial'-`k'' {
							loc str `str' +_b[/b`n']*comb(`n'+`k',`n')*(exp(_b[/lne])*`e'^`n')
						}
						}
