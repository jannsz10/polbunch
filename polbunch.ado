		...
		* This program is part of the polbunch package.
		...
		program polbunch, eclass sortpreserve
			...
			quietly {
				...
				if `estimator'!=0 { //estimate with NLS if estimator>0 + compute string for testing.
					
				//FIND GOOD STARTING VALUES FROM THE UNRESTRICTED MODEL

				if "`log'"=="" {
					loc shiftinit=(_b[/g1]/_b[/b1])^(1/2)-1
				...
				****** specify NL model string *****
				...
				forvalues b=1/`=`H'+`L'' {
					...
					loc init `init' bunch`b' `=_b[/bunch`b']'
					}
				...
				if `estimator'==3 {
					if "`log'"=="" {
						loc shift ((_b[/g0]/_b[/b0])-1)
						forvalues k=1/`polynomial' {
							loc teststr `teststr' (_b[/g`k'] -(1+`shift')^(`k'+1) * _b[/b`k'])
						}
					}
					else {
						loc shift (exp((_b[/g`=`polynomial'-1']-_b[/b`=`polynomial'-1'])/(`polynomial'*_b[/b`polynomial']))-1)
						loc teststr `teststr' (_b[/b`polynomial']-_b[/g`polynomial'])
						if `polynomial'>1 {
							loc str _b[/b`k']
							forvalues k=0/`=`polynomial'-2'' {
								forvalues n=`=`k'+1'/`polynomial' {
									loc str `str' + _b[/b`n']*comb(`n',`k')*ln(1+`shift')^(`n'-`k')
								}
							loc teststr `teststr' (`str')
							}
						}
					}
					loc B=0
					...
					forvalues k=0/`polynomial' {
						if "`log'"=="" loc intstr `intstr' -(_b[/b`k']/(`k'+1))*(((1+`shift')*`cutoff')^(`k'+1)-`cutoff'^(`k'+1))
						else loc intstr `intstr' - ((_b[/b`k']/(`k'+1))*((`cutoff'+ln(1+`shift'))^(`=`k'+1')-`cutoff'^`=`k'+1'))
					}
					loc teststr `teststr' (`intstr')	
				}
				else if `estimator'==2 {
					loc shift ((_b[/g0]/_b[/b0])-1)
					forvalues k=1/`polynomial' {
						loc teststr `teststr' (_b[/g`k']-_b[/b`k']/(1+`shift'))
					}
					loc B=0
					forvalues b=1/`=`H'+`L'' {
						loc B `B'+_b[/bunch`b']
					...
					loc zmax=r(max)+`bw'/2				
					forvalues k=1/`polynomial' {
						loc cns `cns' - (_b[/b`k']/`=`k'+1')*(`zmax'^`=`k'+1'-`cutoff'^`=`k'+1')
					}
					loc teststr `teststr' (`cns')
				}
				else {
					forvalues k=0/`polynomial' {
				...
				nlcom `teststr', post
				forvalues k=1/`=e(rank)' {
					loc test `test' _nl_`k'=
				}
				test `test'=0
				loc chi2=r(chi2)
				loc df=r(df)
