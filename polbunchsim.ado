program polbunchsim, rclass
	...
	noi polbunch z, cutoff(`zstar') pol(`pol') bw(`bw') t0(`t0') t1(`t1') `log' `transform' `noisily' `positiveshift'
	if "`transform'"=="notransform" {
		nlcom (elasticity: ln(1+exp(_b[shift:lnshift]))/(ln(1-`t0')-ln(1-`t1'))), post
		return scalar e=_b[elasticity]
	} 
	else return scalar e=_b[bunching:elasticity]
	noi test elasticity=`el'
	return scalar p=r(p)
	}
end