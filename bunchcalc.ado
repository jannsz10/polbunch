...
program bunchcalc, rclass
	...
	if "`nlcom'"=="" {
		...
		if `estimator'==3  { //nlcom also  shift, MR, el	
			...
			//response of marginal buncher + elasticity
			if "`constant'"=="" {
				if "`log'"!="log" loc nlcom `nlcom' (`shift'*`cutoff')
				else loc nlcom `nlcom' (ln(1+`shift')*`cutoff')
				if "`t0'"!=""&"`t1'"!=""  loc nlcom `nlcom' (ln(1+`shift')/(ln(1-`t0')-ln(1-`t1')))
			}
			else {
				loc nlcom `nlcom' (`bw'*(`B')/(`m'))
				if "`t0'"!=""&"`t1'"!="" loc nlcom `nlcom' (ln((`bw'*(`B')/(`m'))/`cutoff'+1)/(ln(1-`t0')-ln(1-`t1'))) 
				}
			}
			...
			else loc nlcom `nlcom' ((exp(`cutoff'+(`bw'*(`B'))/(`m'))-exp(`cutoff'))/exp(`cutoff'))
			loc nlcom `nlcom' ((`bw'*(`B'))/(`m'))
			if "`t0'"!=""&"`t1'"!="" loc nlcom `nlcom' (ln((`bw'*(`B')/(`m'))/`cutoff'+1)/(ln(1-`t0')-ln(1-`t1')))
			}
		}
	...
	if "`boot'"=="" {
		...
		if `estimator'<3&"`constant'"=="" {
			...
			if "`log'"=="" {
				...
				if "`t0'"!=""&"`t1'"!="" mat `b'=`b',`=ln(exp(eresp+`cutoff')/exp(`cutoff'))/(ln(1-`t0')-ln(1-`t1'))'	
			}
